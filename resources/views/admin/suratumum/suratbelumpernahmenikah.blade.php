...
<body>
    ...
    <h3 style="text-align: center; text-decoration:underline">SURAT KETERANGAN BELUM PERNAH MENIKAH</h3>
   ...
   {{$data->penduduk->kecamatan}} Kota Bukittinggi. Berdasarkan surat pernyataan orang tua yang bersangkutan yang bernama {{ $data->penduduk->nama }}
    diatas materai 6000 pada tanggal {{ \Carbon\Carbon::create($data->create)->format('d-m-Y') }} dan diketahui oleh ketua RT.{{ $data->penduduk->rt }} RW.{{ $data->penduduk->rw }} Kel.{{ $data->getkodeDesa->nama_kel_desa }}
    bahwa yang bersangkutan sampai saat ini belum pernah menikah.</p>
    <p style="text-indent: 0.5in; line-height: 130%">Demikianlah surat keterangan ini di keluarkan sebagai persyaratan {{ $data->tujuan }}.</p>
